...
try:
    ...
    print("TF build info:", build_info)
except Exception as e:
    print("Could not get TF build info:", e)

gpus = tf.config.list_physical_devices('GPU')
print("Detected GPUs (tf):", gpus)
...
if gpus:
    try:
        ...
        # optionally make only the first GPU visible:
        tf.config.set_visible_devices(gpus[0], 'GPU')
        print("Using GPU:", gpus[0])
    except RuntimeError as e:
        print("Could not set GPU configuration:", e)
else:
    print("No GPU detected by TensorFlow — fallback to CPU.")
...
with col1:
    pass
with col2:
    # scanning using a non-blocking Streamlit placeholder (no cv2 GUI windows)
    ...
    if scan_btn and not st.session_state.scanning:
        ...
        if not cap.isOpened():
            ...
            with st.spinner("Scanning emotions..."):
                while frame_count < max_frames:
                    ...
                    for (x, y, w, h) in faces:
                        # draw rectangle and predict on the small face ROI
                        cv2.rectangle(frame, (x, y - 50), (x + w, y + h + 10), (255, 0, 0), 2)
                        roi_gray = gray[y:y + h, x:x + w]
                        try:
                            cropped_img = cv2.resize(roi_gray, (48, 48))
                            cropped_img = np.expand_dims(cropped_img, -1)
                            cropped_img = np.expand_dims(cropped_img, 0)/ 255.0
                            prediction = model.predict_on_batch(cropped_img)
                        except Exception:
                            try:
                                inp_t = tf.convert_to_tensor(cropped_img, dtype=tf.float32) / 255.0
                                prediction = model(inp_t, training=False).numpy()
                            except Exception as e:
                    ...
                    # Convert BGR->RGB for Streamlit and show reduced-size frame to save CPU
            ...
            if list:
                ...
                st.info("No faces detected during scan.")
with col3:
    pass
